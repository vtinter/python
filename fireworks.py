...
class Firework:
    def __init__(self):
        ...
        self.exploded = False
        self.particles = []
        self.min_max_particles = vector(200, 400)  # Thời gian sống tối thiểu và tối đa của pháo hoa đã nổ
    ...
    def update(self, win):
        ...
        if not self.exploded:
            ...
            # Cập nhật trạng thái của các hạt pháo hoa đã nổ
    ...
    def explode(self):
        # Tạo ra các hạt pháo hoa khi pháo hoa cháy nổ
    ...
    def remove(self):
        # Kiểm tra xem pháo hoa đã nổ có cần được loại bỏ hay không
        if self.exploded:
...
class Particle:
    ...
    def apply_force(self, force):
        # Áp dụng lực lên hạt pháo hoa
        self.acc += force

    def move(self):
        # Di chuyển hạt pháo hoa
    ...
    def show(self, win):
        # Hiển thị hạt pháo hoa
        pygame.draw.circle(win, (self.colour[0], self.colour[1], self.colour[2], 0), (int(self.pos.x), int(self.pos.y)),
                           self.size)

    def decay(self):
        # Phân rã hạt pháo hoa theo thời gian sống
        if 100 > self.life > 20:  # Thời gian sống của hạt pháo hoa
            ran = randint(0, 30)
            if ran == 0:
                self.remove = True
        elif self.life > 100:  # Thời gian sống của hạt pháo hoa
            ran = randint(0, 5)
            if ran == 0:
                self.remove = True

    def trail_update(self):
        # Cập nhật vị trí của dấu vết theo vị trí trước đó
        self.prev_posx.pop()
...
fireworks = [Firework() for i in range(5)]  # Tạo ra 5 pháo hoa ban đầu
running = True
...
while run:
    ...
    if randint(0, 80) == 20:
        num_fireworks = randint(1, 3)  # Số lượng pháo hoa mới được thêm vào
        fireworks.extend([Firework() for _ in range(num_fireworks)])
